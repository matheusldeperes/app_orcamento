...
# --- CAPTURA DE FOTOS ---
# Removido o argumento facing_mode para evitar o TypeError
foto_capturada = st.camera_input("Tirar foto")

if foto_capturada:
    # Salvamos a foto na lista do estado da sessão
    st.session_state.lista_fotos.append(foto_capturada)
    # O rerun limpa o componente camera_input, permitindo tirar a próxima foto imediatamente
    st.rerun()

if st.session_state.lista_fotos:
    st.write(f"Fotos capturadas: {len(st.session_state.lista_fotos)}")
    cols = st.columns(3)
...
if not st.session_state.finalizado:
    if st.button("Gerar Orcamento", use_container_width=True):
        if os_num and st.session_state.lista_fotos:
            ...
            st.warning("Preencha a OS e tire fotos antes de finalizar.")
